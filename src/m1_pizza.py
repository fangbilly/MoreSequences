...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(155.0, 50.0),  # All numbers are approximate.
                rg.Point(72.5, 97.6),
                rg.Point(72.5, 2.4)]
    circle = rg.Circle(rg.Point(100, 50), 55)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    dot_list = generate_points_on_circle(circle, number_of_points)
    for dots in dot_list:
        small_circle = rg.Circle(dots, 10)
        small_circle.fill_color = color
        small_circle.attach_to(window)
        points = small_circle.center
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # Test 4:
    title = 'PIZZA test 4:  200 slices, thin (thickness=1) blue lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 1
    pizza(window, circle, 200, 'blue', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dot_list = generate_points_on_circle(circle, number_of_slices)
    for dots in dot_list:
        line = rg.Line(circle.center, dots)
        line.color = color
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    # Tests 3:
    title = 'POLYGON tests 3:  5 medium green lines.'
    window = rg.RoseWindow(550, 400, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dot_list = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(dot_list)):
        start = k
        end = k+1
        if end == len(dot_list):
            end = 0
        line = rg.Line(dot_list[start], dot_list[end])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 300, 61, 'lime green', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    dot_list = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(dot_list)):
        start = dot_list[k % len(dot_list)]
        end = dot_list[(k+hops_to_next_point) % len(dot_list)]
